		...
		<tr>
			...
			<td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
				<p className='text-gray-900 whitespace-no-wrap'>
					{moment.unix(item.txIns[item.txIns.length - 1].timestamp).fromNow()}
				</p>
			</td>
			<td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
				<p className='text-gray-900 whitespace-no-wrap'>
			...
			<td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
				<p className='relative text-blue-400 truncate w-64'>
				...
				<div className='-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 h-full overflow-x-auto overflow-y-scroll'>
					<div className='inline-block min-w-full shadow rounded-lg overflow-hidden'>
						<table className='min-w-full leading-normal'>
							...
							<tbody>
								{data.length > 0 && data.map((item) => <TxItem item={item} />)}
							</tbody>
						</table>
					</div>
				</div>
